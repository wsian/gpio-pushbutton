...
# define num 1                                      // Number of PINs used

unsigned int nums = 1;                                // Number of PINs used again
unsigned int WpiPinsSelection[] = {25};   // List of GPIO used by buttons in Wpi semantics
...
void init_gpio()
{
  ...
  {
    //lws printf("init WiringOP GPIO %d - Physical Opi PC Pinout : %d \n", WpiPinsSelection[i], find_OpiPin(WpiPinsSelection[i]));
...
void logic()
{
  if (act_inputs == 0)   // === If there are no active inputs ...
  {
    ...
    for (i = 0; i < nums; i++)
    {
      ...
      {
        //lws printf("----- info : PIN %d has been activate during %d periods ----- \n", WpiPinsSelection[i], CountTime);
  ...
  else if (act_inputs == 1)    // === If there is one active input - start comparison
  {
    ...
    for (i = 0; i < nums; i++)
    {
      ...
      {
        old_state[i] = curr_state[i];
        //lws printf("Never Ever ! Never Ever ! Never Ever ! Never Ever ! Never Ever !");
      }
